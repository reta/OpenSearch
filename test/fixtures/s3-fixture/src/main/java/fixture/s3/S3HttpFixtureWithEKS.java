...
public class S3HttpFixtureWithEKS extends S3HttpFixture {
    ...
    protected HttpHandler createHandler(final String[] args) {
        ...
        final String eksRoleArn = Objects.requireNonNull(args[4]);
        final String eksRoleSessionName = Objects.requireNonNull(args[5], "role session name is missing");
        final HttpHandler delegate = new S3HttpHandler(bucket, basePath);
        ...
        return exchange -> {
            ...
            if ("POST".equals(exchange.getRequestMethod()) && exchange.getRequestURI().getPath().startsWith("/eks_credentials_endpoint")) {
                final byte[] response = buildCredentialResponse(eksRoleArn, eksRoleSessionName).getBytes(StandardCharsets.UTF_8);
    ...
    protected String buildCredentialResponse(final String roleArn, final String roleSessionNamee) {
