 ...
 * @author kimchy
 */
public class S3FileEntrySettingsTests extends TestCase {

    public void testDefaultSettings() throws Exception {
        final S3FileEntrySettings settings = new S3FileEntrySettings();

        assertEquals(3, settings.getProperties().size());
        assertEquals(ActualDeleteFileEntryHandler.class, settings.getSettingAsClass(S3FileEntrySettings.FILE_ENTRY_HANDLER_TYPE, null));
        assertEquals(FetchOnBufferReadS3IndexInput.class, settings.getSettingAsClass(S3FileEntrySettings.INDEX_INPUT_TYPE_SETTING, null));
        assertEquals(RAMS3IndexOutput.class, settings.getSettingAsClass(S3FileEntrySettings.INDEX_OUTPUT_TYPE_SETTING, null));
    }
