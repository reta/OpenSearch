...
public class S3Directory extends Directory {
    ...
    private void initialize(final String bucket, final String shard, S3DirectorySettings settings) {
        ...
        for (final String name : fileEntrySettings.keySet()) {
            ...
            try {
                final Class<?> fileEntryHandlerClass = feSettings.getSettingAsClass(S3FileEntrySettings.FILE_ENTRY_HANDLER_TYPE, null);
                ...
                throw new IllegalArgumentException(
                    "Failed to create FileEntryHandler  ["
    ...
    public boolean bucketExists() {
        try {
            ...
            SocketAccess.doPrivilegedVoid(() -> s3.headBucket(b -> b.bucket(bucket)));
            return true;
    ...
    public void create() {
        if (!bucketExists()) {
            ...
            SocketAccess.doPrivilegedVoid(() -> s3.createBucket(b -> b.bucket(bucket).objectLockEnabledForBucket(true)));
        }
        try {
            if (logger.isDebugEnabled()) {
