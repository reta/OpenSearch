...
public class FetchOnBufferReadS3IndexInput extends S3BufferedIndexInput {
    ...
    protected void refill() throws IOException {
        ...
        ResponseInputStream<GetObjectResponse> res =  
                SocketAccess.doPrivileged(() -> s3Directory.getS3().getObject(b -> b.bucket(s3Directory.getBucket()).key(name)));
    ...
    protected synchronized void readInternal(final byte[] b, final int offset, final int length) throws IOException {
        ...
        ResponseInputStream<GetObjectResponse> res = SocketAccess.doPrivileged(() -> 
            s3Directory.getS3().getObject(bd -> bd.bucket(s3Directory.getBucket()).key(name)));
